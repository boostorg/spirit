...
namespace boost { namespace spirit { namespace x3
{
    ...
    // optional/reference_wrapper interfaces. Usage:
#if 0
    struct my_error_dispatcher
    {
        // Note that `Exception` can be any of
        //   - C++ exception (i.e. `expectation_failure<Iterator> : std::runtime_error`)
        //   - Optional (i.e. `optional<expectation_failure<Iterator>>`)
        //   - Reference to optional (i.e. `reference_wrapper<optional<expectation_failure<Iterator>>>`)
        ...
        x3::error_handler_result on_error(
            ...
            auto& my_error_handler = x3::get<x3::error_handler_tag>(context).get();
            my_error_handler.print("Error! Expecting " + x3::which(x) + " here:", x3::where(x));
        }
    };
#endif
